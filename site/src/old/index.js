...
import('../node_modules/sudoku/sudoku.js').then((sudoku) => {
  const new_sudoku = document.getElementById('new')
...
const middle_cells = new Array()
const corner_cells = new Array()
const background_cells = new Array()
const solution_count = document.getElementById('count')
/* Clue or solve */
let app_mode = 'setter'
const app_mode_text = document.getElementById('app_mode')

/* Cell mouse mode */
let mouse_mode = 'selection'
const mouse_add_map = new Map()
const mouse_finish_click_map = new Map()
const mouse_clear_map = new Map()
const mouse_render_map = new Map()
const mouse_mode_leave_map = new Map()

/* Side line mouse mode */
let side_mouse_mode = null
const side_mouse_click = new Map()
const side_mouse_render = new Map()

/* Solver keyboard_mode */
let keyboard_mode = 'normal'
const key_number_button = document.getElementById('key_number')
const key_middle_button = document.getElementById('key_middle')
const key_corner_button = document.getElementById('key_corner')
const key_color_button = document.getElementById('key_color')

const key_number = document.getElementById('number')
const key_middle = document.getElementById('middle-number')
const key_corner = document.getElementById('corner-number')
const key_color = document.getElementById('color')

const setter_side = document.getElementById('setter-side')
const selected = new Set()
const input = document.getElementById('save')
let diag_pos = false
const diag_pos_vis = document.getElementById('diag_pos_vis')
let diag_neg = false
const diag_neg_vis = document.getElementById('diag_neg_vis')
let king = false

const diag_pos_button = document.getElementById('diag_pos_button')
const diag_neg_button = document.getElementById('diag_neg_button')
const king_button = document.getElementById('king_button')

const solving_a = document.getElementById('solving_url')
const setting_a = document.getElementById('setting_url')
const sudokuwiki = document.getElementById('sudokuwiki')

// Settings
const setter_settings = new Settings()
...
// Puzzle information
const puzzle_name = document.getElementById('puzzle_name')
const puzzle_name_edit = document.getElementById('puzzle_name_edit')
const puzzle_author = document.getElementById('puzzle_author')
const puzzle_author_edit = document.getElementById('puzzle_author_edit')
const puzzle_message = document.getElementById('puzzle_message')
const puzzle_message_edit = document.getElementById('puzzle_message_edit')
const puzzle_variant_rule = document.getElementById('puzzle_variant_rule')

// Thermo related vars
const thermo_surround_button = document.getElementById('thermo')
const thermo_edit = thermo_surround_button.children[0]
const thermo_delete = thermo_surround_button.children[1]
let thermo_data = new Array()
const thermo_svg = document.getElementById('thermo_svg')

// Difference related vars
const difference_button = document.getElementById('difference')
let difference_data = new Array()
const difference_svg = document.getElementById('difference_svg')

// Solve button
const solve = document.getElementById('solve')
const solve_result_message = document.getElementById('solve_result_message')
const solve_result_message_text = document.getElementById(
  'solve_result_message_text'
)

function render_selected () {
  for (let i = 0; i < cells.length; i++) {
    const full_cell = cells[i].parentElement
    if (selected.has(i)) {
...
function mouse_clear (index, shift) {
  const f = mouse_clear_map.get(mouse_mode)
  if (f) {
    f(index, shift)
  }
}
...
function mouse_add (index) {
  const f = mouse_add_map.get(mouse_mode)
  if (f) {
    f(index)
  }
}
...
function mouse_render (index) {
  const f = mouse_render_map.get(mouse_mode)
  if (f) {
    f(index)
  }
}
...
function init_cells () {
  ...
  const sudoku_ul = document.createElement('ul')
  ...
  for (let i = 0; i < 81; i++) {
    const cell_li = document.createElement('li')
    ...
    const middle_cell_span = document.createElement('span')
    cell_li.appendChild(middle_cell_span)
    middle_cells.push(middle_cell_span)
    middle_cell_span.classList.add('middle-cell')

    const corner_in_cell = new Array()
    corner_cells.push(corner_in_cell)
    for (const id in corner_names) {
      const corner_name = corner_names[id]
      const corner_cell_span = document.createElement('span')
    ...
    // On the bottom
    if (i < 72) {
      const bottom_cell = document.createElement('span')
  ...
  document.addEventListener('mouseup', () => {
    is_mouse_down = false
    const f = mouse_finish_click_map.get(mouse_mode)
    if (f) {
      f()
      mouse_render()
    }
...
function cell_side_click (e) {
  ...
  let f = side_mouse_click.get(side_mouse_mode)
  if (f) {
    f(e.target)
...
function toggle_mouse_mode (target, if_already) {
  const f = mouse_mode_leave_map.get(mouse_mode)
  if (f) {
    f()
    mouse_render()
  }
...
function init_button () {
  ...
  const cc_button = document.getElementById('clear_computer')
  cc_button.onclick = clear_computer
  const clear_button = document.getElementById('clear')
  clear_button.onclick = clear
  const reset_button = document.getElementById('reset')
  reset_button.onclick = reset
  const app_mode_button = document.getElementById('app_mode_button')
  ...
  const keyboard_buttons = Array.from(key_number.children).concat(
    Array.from(key_middle.children)
  )
  for (let i = 0; i < keyboard_buttons.length; i++) {
    const button = keyboard_buttons[i]
    button.onclick = () => {
...
function toggle_cell_color (cell_id, color_index) {
  ...
  const cell = cells[cell_id].parentNode
  let colors = background_cells[cell_id]
  let style = ''
  if (color_index != 'Delete' && color_index != 'Backspace') {
    const pos = colors.indexOf(color_index)
    if (pos >= 0) {
      colors.splice(pos, 1)
...
function update_variant_visual () {
  ...
  // Complex variants
  let thermo_cl = thermo_edit.classList
  ...
  // Difference button
  const difference_cl = difference_button.classList
  ...
  let found = update_mode('middle', key_middle_button, key_middle)
...
function handle_key_event (key) {
  ...
  const current_kind = app_mode == 'setter' ? 'clue' : 'human'
...
function get_line_with (selector) {
  ...
  for (let i = 0; i < 81; i++) {
    const cell = cells[i]
    const content = cells[i].innerHTML
...
function get_variant () {
  ...
  for (const t_id in thermo_data) {
    const thermo = thermo_data[t_id]
    line += ';thermo'
    for (const n_id in thermo) {
      const number = thermo[n_id]
      line += '|' + number
    }
  }
  for (const id in difference_data) {
    const d = difference_data[id]
...
function get_save_data (include_human = true) {
  const clue = 'clue' + get_line_with((cell) => cell.classList.contains('clue'))
  let human = ''
  if (include_human) {
    human =
...
function update_url () {
  const base = get_url() + '?data='
  const new_url = base + get_save_data() + solve_param()
    ...
    case 'corner':
      ...
      if (value != '') {
        ...
        for (pos_id in corner_cells[id]) {
          const pos_content = corner_cells[id][pos_id].innerHTML
...
function draw_thermo (numbers) {
  let n = numbers[0]
  let [line, col] = svg_pos_from_id(n)
...
function start_new_thermo (index) {
  const new_thermo = new Array()
  new_thermo.push(index)
  thermo_data.push(new_thermo)
}

function add_thermo (cell) {
  const current_thermo = thermo_data[thermo_data.length - 1]
  const is_index_in_thermo = current_thermo.indexOf(cell) != -1
  if (current_thermo.length < 9 && !is_index_in_thermo) {
    const last_cell = current_thermo[current_thermo.length - 1]
    const lc_x = last_cell % 9
    const lc_y = Math.trunc(last_cell / 9)
    const c_x = cell % 9
    const c_y = Math.trunc(cell / 9)
...
function remove_thermo (index) {
  for (let id = thermo_data.length - 1; id >= 0; id--) {
    const found_index = thermo_data[id].indexOf(index)
...
function remove_one_cell_thermo () {
  for (let i = 0; i < thermo_data.length; i++) {
...
function difference_click (element) {
  const cell1 = element.getAttribute('cell1')
  const cell2 = element.getAttribute('cell2')
  const dir = element.getAttribute('right')
...
function draw_difference (difference) {
  const c1 = difference[0]
  const c2 = difference[1]
  const [l1, o1] = svg_pos_from_id(c1)
  const [l2, o2] = svg_pos_from_id(c2)
...
function handle_sudoku_message (message) {
  ...
  if (message[0] == 'solve_count') {
    ...
    const cs = message[1].split(';')
    let solve_result = null
    if (cs.length == 1) {
      solve_result = cs[0]
    } else {
      const max_middle = setter_settings.get_number('solve_max_middle_number', 4)
      let all_cell_too_much_poss = true
      for (let i = 0; i < 81; i++) {
        const c = cs[i]
        if (c.length == 0) {
...
const par_solve_only = params.get('solve')
